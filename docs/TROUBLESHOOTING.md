...
**Status:** Abandoned by design (v1.1.2). DoD enforcement consolidated at the closure review via the `dod-reviewer` sub-agent — since v1.2.0 the final Done-when item of every goal (previously the final task of each iteration plan). The hook layer was dropped entirely — three layers (hook reminder, per-commit DoD, closure-review DoD) created ambiguity, and the closure review is the one that actually catches gaps. Plugin v1.1.2 has no `hooks/` directory.
