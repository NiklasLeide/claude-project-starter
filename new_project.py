...
def create_shared_conventions(cfg):
    """Write .claude/shared-conventions.md into the generated project."""
    d = cfg["target_dir"]
    write_file(os.path.join(d, ".claude", "shared-conventions.md"), """\
# Shared Conventions
...
def create_agents(cfg):
    """Write sub-agent definitions to .claude/agents/ in the generated project."""
    d = cfg["target_dir"]
    agents_dir = os.path.join(d, ".claude", "agents")
    Path(agents_dir).mkdir(parents=True, exist_ok=True)
    header("Creating Claude Code sub-agents")

    write_file(os.path.join(agents_dir, "dod-reviewer.md"), """\
---
name: dod-reviewer
...
""")
    ...
    write_file(os.path.join(agents_dir, "code-researcher.md"), """\
---
...
def create_settings_and_hook(cfg):
    """Write .claude/settings.json and session-start-reminder.txt into the generated project."""
    d = cfg["target_dir"]
    header("Creating Claude Code settings and session hook")
            ...
            "SessionStart": [
                {
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": 'cat "$CLAUDE_PROJECT_DIR/.claude/session-start-reminder.txt" 2>/dev/null || true'
                        }
                    ]
                }
            ]
    ...
    write_file(os.path.join(d, ".claude", "session-start-reminder.txt"), """\
============================================================
Definition of Done — verified at sprint close:
  1. Code works (manually verify happy path)
  2. Tests written (new behavior has coverage)
  3. Existing tests pass (run the test command, zero failures)
  4. docs/CHANGELOG.md updated (one line per logical change)
  5. ./commit.sh "message" run successfully

Every sprint plan ends with a "Run DoD review" task — use the
dod-reviewer sub-agent when you reach it.

Full conventions: @.claude/shared-conventions.md
============================================================
""")
...
When drafting the sprint's task table, include a final row:
| N.X | Run DoD review for this sprint | ⬜ Todo | Use dod-reviewer sub-agent |
This is mandatory per shared-conventions.md — every sprint must end with
a DoD review.
...
{C.BOLD}Sub-agents:{C.RESET}
  {C.CYAN}dod-reviewer{C.RESET}     Invoked at sprint close to verify Definition of Done
  {C.CYAN}code-researcher{C.RESET}  Tactical research when Claude Code hits unfamiliar APIs

{C.BOLD}Automation:{C.RESET}
  {C.CYAN}.claude/settings.json{C.RESET}           SessionStart hook shows DoD reminder
...
def update_project(target_dir):
    ...
    # Re-generate shared conventions, sub-agents, settings, workflow, and slash commands
    create_shared_conventions(cfg)
    create_agents(cfg)
    create_settings_and_hook(cfg)
...
''', encoding="utf-8")
    ...
    print(f"  {C.DIM}CLAUDE.md was NOT touched — shared-conventions, sub-agents, settings, workflow, slash commands, and commit.sh were refreshed.{C.RESET}")
