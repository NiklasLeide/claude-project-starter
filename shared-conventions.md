...
## Goal closure (non-negotiable)

Work is defined as goals, not sprints. A goal has four mandatory fields:
**Outcome** (what is true when done), **Done when** (binary verifiable
conditions — machine-checkable or a single human check; "feels done" is
not a condition), **Budget** (cost/effort cap), **Depends on**.

Every goal in `docs/PROJECT_STATUS.md` lists its Done-when conditions as
checkboxes. A goal is closed ONLY by verifying them:

1. Invoke the `dod-reviewer` sub-agent. It reads the goal's Done-when
   list from PROJECT_STATUS.md and verifies each condition plus the DoD
   checklist.
2. If it reports "GOAL NOT CLOSEABLE", address what's unresolved and
   re-run. Do not close a goal with open gaps.
3. If it reports "GOAL READY TO CLOSE":
   - Tick the goal in PROJECT_STATUS.md
   - Add a CHANGELOG line: `[YYYY-MM-DD] goal: [name] closed — [what shipped]`
   - Update "Last updated" in PROJECT_STATUS.md
   - Run `./commit.sh "goal: [name] close"`

There is no sprint cadence. Pick the next goal by friction reduction per
effort. This applies to Claude chat (defines goals) and Claude Code
(executes and closes them). Chat drafting a new goal must include its
Done-when conditions; Claude Code closing a goal must invoke the
sub-agent rather than skipping verification.
...
- Flag scope creep: "do we actually need this in the current goal?"
